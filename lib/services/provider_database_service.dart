...
class ProviderDatabaseService {
  ...
  /// Busca proveedores por sus IDs (usuario_id o id de perfil)
  Future<List<ProviderData>> getProvidersByIds(List<String> ids) async {
    if (ids.isEmpty) return [];

    try {
      // Intentamos buscar por usuario_id primero, ya que es el ID principal que usamos
      ...
      return data
          .map(
            (item) => ProviderData(
              ...
              tipoSuscripcion:
                  item['tipo_suscripcion_actual'] as String? ?? 'basico',
              categoria: item['categoria_principal'] as String? ?? '',
              paquetes: [],
            ),
          )
          .toList();
  ...
  Future<List<ProviderData>> getProvidersByCategoryName(
    String categoryName,
  ) async {
    try {
      // Priorizar búsqueda por categoría de servicio (tabla categorias_servicio)
      final fromServices = await _getProvidersByServiceCategory(categoryName);
      if (fromServices.isNotEmpty) return fromServices;

      // Si no hay resultados, devolver todos los proveedores como fallback
      return await _getAllProvidersForCategory(categoryName);
    } catch (e) {
      // En caso de error, devolver lista vacía para que el UI decida
      return [];
    }
  }
  ...
  Future<List<ProviderData>> _getAllProvidersForCategory(
    String categoryName,
  ) async {
    try {
      final response = await _client
  ...
  Future<List<ProviderData>> _getProvidersByServiceCategory(
    String categoryName,
  ) async {
    try {
